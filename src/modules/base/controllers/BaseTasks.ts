...
export default class BaseTasks {
    getAndCreateTaskInviteLink(taskId: number, role: string): string {
    ...
    async checkRole(role: string | undefined) {
        switch (role) {
            case "read-only":
            case "read-write":
                break;
    ...
    async checkOwner(userId: number, taskId: number) {
